...
public class CreateProjectScreen extends BaseFrame {
    ...
    private void createProject() {
        ...
        try {
            ApiClient.createProject(name, enteredTeams);
            nameField.setText("");
            teamFields.clear();
            addTeamRow();
            loadProjects();
    ...
    private void loadProjects() {
        SwingWorker<List<Map<String, Object>>, Void> worker = new SwingWorker<>() {
            ...
            protected void done() {
                try {
                    List<Map<String, Object>> projects = get();
                    projectsModel.setRowCount(0);
                    for (Map<String, Object> p : projects) {
                        List<String> teams = (List<String>) p.get("teams");
                        String teamsText = teams == null ? "" : String.join(", ", teams);
                        projectsModel.addRow(new Object[]{p.get("id"), p.get("name"), teamsText});
                    }
                } catch (Exception e) {
